...
let parse ~page ?title add_link content source =
  Wiki_syntax.(
...
let write_file ~fn content =
  ...
  output_string ch content; close_out ch

let script_name = "/js/client.js"

let write_script fn = write_file fn [%blob "../client.js"]

let style_name = "/css/style.css"

let write_style fn = write_file fn [%blob "../style.css"]
...
let stylesheets =
  [style_name; "https://cdnjs.cloudflare.com/ajax/libs/prism/1.9.0/themes/prism.min.css"]
...
let render ch ~title:t ~page content =
  ...
  let h =
    Html.(
      ...
      :: meta ~a:[a_content "width=device-width, initial-scale=1"; a_name "viewport"] ()
  ...
  let a_cl =
    ...
    | Document.Project {project; version; page} ->
        ...
        in
        Some [Html.a_class (project :: Version.to_string version :: page_class)]
    | _ -> None
  in
  (Html.pp () fmt @@ Html.(html (head (title (pcdata t)) h) (body ?a:a_cl content)));
...
let extract_h1 blocks =
  let rec f = function
    ...
    (* not found at all *)
  in
  blocks |> List.map Tyxml_html.toelt |> f
