...
(** A wikicreole_parser is essentially a [Wikicreole.builder] object but easily
    extensible. That is, the fields [plugin] and [plugin_action] of
    [Wikicreole.builder], which are supposed to be functions, are here
    represented as association tables. Thus, it becomes easy (and, more
    importantly, not costly) to add extensions. *)
type 'a wikicreole_parser = (module Parser with type res = 'a)

(** Preparser are actually used to rewrite contents of wiki extension when
    storing a wikipage on the database. This is currently used only for creating
    wikibox. *)
type preparser =
...
(** Module type for representing extensible wikicreole parser on which we can
    register wiki extension. *)
module rec ExtParser : sig
  module type ExtParser = sig
    ...
    module type RawWikiPlugin = sig
      ...
      val ni_plugin :
        (rec_res_without_interactive wikicreole_parser -> simple_ni_plugin)
        option
    end
