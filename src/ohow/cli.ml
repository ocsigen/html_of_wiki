...
let outfile_cmd =
  let doc =
    "Writes the HTML into the given file. Always overwrites. Overrides $(b,--print)."
  in
...
let root_cmd =
  ...
  Cmdliner.Arg.(value & opt string (Unix.getcwd ()) & info ["root"] ~docv:"DIR" ~doc)

let manual_cmd =
  let doc = "Use the given manual path." in
  Cmdliner.Arg.(value & opt (some string) None & info ["manual"] ~docv:"DIR" ~doc)

let api_cmd =
  let doc = "Use the given api path." in
  Cmdliner.Arg.(value & opt (some string) None & info ["api"] ~docv:"DIR" ~doc)

let default_subproject_cmd =
  let doc =
    "Mirrors the effect of the \"default_subproject\" feature in Ocsiforge's old \
     config.js for compatibility reasons. Do not use it otherwise!"
  in
  Cmdliner.Arg.(
    value & opt (some string) None & info ["default-subproject"] ~docv:"NAME" ~doc)

let img_cmd =
  let doc = "Use the given image directory path." in
  Cmdliner.Arg.(value & opt (some string) None & info ["images"] ~docv:"DIR" ~doc)

let assets_cmd =
  let doc = "Use the given assets directory path." in
  Cmdliner.Arg.(value & opt (some string) None & info ["assets"] ~docv:"DIR" ~doc)

let template_cmd =
  let doc = "Use the given template." in
  Cmdliner.Arg.(value & opt (some file) None & info ["t"; "template"] ~docv:"FILE" ~doc)
...
let csw_cmd =
  ...
  Cmdliner.Arg.(
    value
    & opt (some file) None
    & info ["csw"; "clientserverswitch"; "client-server-switch"] ~docv:"FILE" ~doc)
...
let local_cmd =
  let doc =
    "Appends `.html' at the end of each link redirection to make local navigation using \
     file:// URLs possible."
  in
...
let info_cmd =
  Cmdliner.(
    ...
    let man =
      ...
      ; `P "$(tname) is a command line utility for compiling Wikicreole files into HTML."
      ; `S "EXTENSIONS"
      ; `P
      ...
      ; `P
          "Expands to a link to the `chapter' page inside `project''s manual directory \
           and for a given `version'. A `fragment' can be added. Default values:"
      ...
      ; `P
          "Expands to a link to the `thing' of the given `project' or/and `subproject' \
           of the given `version'. The link's text can be chosen using the `text' \
      ...
      ; `S "LINKS"
      ; `P
          "$(tname) is supposed to compile each wiki independently but these might \
           contain links. Since $(tname) is not responsible for checking for dead \
           ...
           information."
      ; `P
          "- All the projects which links to each other must be placed inside the same \
           directory."
      ; `P
          "- Inside each project directory are the root directories of each version of \
           the documentation."
      ; `P
          "- The architecture inside each root (version of project) directory is less \
           strict. However, if the a_manual tag is used, then the manual directory must \
           not contain wikis in sub-directories."
      ; `P
          "- If any of the a_api* tags is used, then each sub-project directory - if \
           any - must be located inside the api directory and every wiki must be \
           directly inside the api directory or its sub-project directory (i.e: no \
           sub-directories are allowed)."
      ; `P
          "The options $(b,--root), $(b,--manual) and $(b,--api) can be used to \
           explicitly provide the path to the, respectively, the root directory, the \
