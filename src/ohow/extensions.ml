...
let get_opts ?defaults opts args =
  ...
  defaults
  <$> (fun def ->
        List.map2 (fun sx -> function Some d -> Some (sx |? d) | None -> sx) values def
        )
  |? values
...
let _reg name f =
  ...
  Wiki_syntax.register_raw_wiki_extension
    ...
    ~wp_rec
    (fun _ -> Error.wrap_phrasing name f );
  Wiki_syntax.register_raw_wiki_extension
    ~name
...
let register ?defaults name opts f = _reg name @@ extension_with_opts ?defaults opts f
